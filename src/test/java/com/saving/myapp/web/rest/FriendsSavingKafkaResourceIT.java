...
import com.saving.myapp.config.KafkaProperties;
...
class FriendsSavingKafkaResourceIT {
    ...
    void setup() {
        ...
        FriendsSavingKafkaResource kafkaResource = new FriendsSavingKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/friends-saving-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/friends-saving-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
